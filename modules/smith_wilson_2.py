...
# --- MOTEUR DE CALCUL SMITH-WILSON (LOGIQUE RÉELLE) ---

def wilson_kernel(t, u, alpha, ufr):
    """
    Implémentation du noyau de Wilson tel que défini par l'EIOPA.
    t, u : maturités
    alpha : paramètre de convergence
    ufr : Ultimate Forward Rate (log-complété)
    """
    exp_ufr = np.exp(-ufr * (t + u))
    min_tu = np.minimum(t, u)
    max_tu = np.maximum(t, u)
    
    # Formule exacte du noyau W(t,u)
    kernel = exp_ufr * (alpha * min_tu - 0.5 * np.exp(-alpha * max_tu) * (np.exp(alpha * min_tu) - np.exp(-alpha * min_tu)))
    return kernel

def compute_smith_wilson(t_target, t_market, r_market, alpha, ufr_input):
    """
    Résout le système matriciel et projette la courbe.
    """
    # Conversion de l'UFR (taux annuel composé en taux continu)
    ufr = np.log(1 + ufr_input)
    
    # 1. Facteurs d'actualisation observés (Prix de marché)
    p_market = 1 / (1 + r_market)**t_market
    
    # 2. Valeur cible (Cible d'extrapolation)
    mu = np.exp(-ufr * t_market)
    
    # 3. Construction de la matrice de noyau W
    N = len(t_market)
    W = np.zeros((N, N))
    for i in range(N):
        for j in range(N):
            W[i, j] = wilson_kernel(t_market[i], t_market[j], alpha, ufr)
            
    # 4. Résolution du système linéaire : W * zeta = p - mu
    # zeta représente les poids de chaque point de marché dans l'extrapolation
    zeta = np.linalg.solve(W, p_market - mu)
    
    # 5. Projection sur les maturités cibles
    results = []
    for t in t_target:
        term_sum = sum(zeta[i] * wilson_kernel(t, t_market[i], alpha, ufr) for i in range(N))
        p_t = np.exp(-ufr * t) * (1 + term_sum)
        # Reconversion du prix en taux annuel actuariel
        r_t = (1 / p_t)**(1/t) - 1
        results.append(r_t)
...
with check_col1:
    ...
    val_10y = compute_smith_wilson(np.array([10.0]), t_market, r_market, alpha_val, ufr_val)[0]
